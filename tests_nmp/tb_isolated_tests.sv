...
// Run each test in isolation to avoid any state machine conflicts
module tb_isolated_tests;
        ...
        begin
            ...
            if (inter_flag) begin
                ...
                if (errors == 0) begin
                    ...
                    $display("  FAIL: %s (%0d errors)", test_name, errors);
    ...
    initial begin
        ...
        run_single_test("All zeros",   17'h0007, 5'd4,  32'h1000, 32'h2000, 32'h3000, 32'h000, 32'h000);
        run_single_test("Max+1",       17'h0008, 5'd2,  32'h1000, 32'h2000, 32'h3000, 32'hFFFFFFFF, 32'h1);
        
        $display("\n=== SUMMARY ===");
        $display("Total: %0d, Passed: %0d, Failed: %0d, Rate: %0d%%", 
                 test_count, pass_count, fail_count, (pass_count*100)/test_count);
