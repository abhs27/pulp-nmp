...
module tb_nmp_hash_debug2;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (axia_arvalid && axia_arready) begin
                ...
                axia_rvalid <= 1;
                axia_rresp  <= 2'b00;
                $display("Time=%0t: AXI-A READ addr=0x%08X data=0x%08X", $time, axia_araddr, memory[axia_araddr[11:2]]);
            ...
            if (axib_arvalid && axib_arready) begin
                ...
                axib_rvalid <= 1;
                axib_rresp  <= 2'b00;
                $display("Time=%0t: AXI-B READ addr=0x%08X data=0x%08X", $time, axib_araddr, memory[axib_araddr[11:2]]);
            ...
            if (axic_awvalid && axic_awready && axic_wvalid && axic_wready) begin
                ...
                axic_bvalid <= 1;
                axic_bresp  <= 2'b00;
                $display("Time=%0t: AXI-C WRITE addr=0x%08X data=0x%08X", $time, axic_awaddr, axic_wdata);
    ...
    // Monitor signals
    always @(posedge clk) begin
        if (nmp_active && dut.current_state == 3) begin // EXECUTE_READ
            $display("Time=%0t: EXECUTE_READ - start_element_read=%b, rs1_read_done=%b, rs2_read_done=%b, rs1_base=0x%08X, rs2_base=0x%08X",
                     $time, dut.start_element_read, dut.rs1_read_done, dut.rs2_read_done, dut.rs1_base_addr, dut.rs2_base_addr);
        end
    end
    ...
    initial begin
        ...
        $display("\n=== DEBUG TESTBENCH 2 - AXI Monitoring ===\n");
        ...
        $display("[3] Executing instruction: 0x%08X\n", instruction);
        ...
        if (inter_flag) begin
            $display("\n[4] ✓ PASS: Operation completed!");
        end else begin
            $display("\n[4] ✗ FAIL: Timeout - FSM in state %0d", dut.current_state);
        end
